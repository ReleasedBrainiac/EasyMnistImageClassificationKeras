...
class DatasetProvider(object):
    ...
    def LoadMnistNumberImages(self, verbose:int = 0):
        try:
            if (verbose > 0): print("Load mnist number images dataset split in train and test set!")
            (x_train, y_train), (x_test, y_test) = keras.datasets.mnist.load_data()

            if (verbose > 0): print("Rescale images range to [0, 1]")
            x_train = x_train.astype("float32") / 255
            x_test = x_test.astype("float32") / 255

            if (verbose > 0): print("Expand images with shape " + str(x_train.shape) + " to next higher dimension.")
